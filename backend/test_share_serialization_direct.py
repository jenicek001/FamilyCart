...
async def test_share_serialization():
    ...
    async for session in get_session():
        try:
            ...
            print(f"✅ Found shopping list: {shopping_list.name} with {len(shopping_list.items)} items")
            
            # Test 1: Try to serialize the shopping list directly
            print("\n🧪 Test 1: Direct ShoppingListRead serialization")
            try:
                list_schema = ShoppingListRead.model_validate(shopping_list, from_attributes=True)
            ...
            try:
                ...
                result_schema = ShoppingListRead.model_validate(result, from_attributes=True)
                result_json = result_schema.model_dump(mode='json')
                print(f"✅ Share result serialization successful")
                print(f"   Members after sharing: {len(result_json.get('members', []))}")
