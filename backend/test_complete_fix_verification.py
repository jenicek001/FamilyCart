...
async def authenticate_user(session: aiohttp.ClientSession, email: str, password: str) -> str:
    ...
    async with session.post(f"{BASE_URL}/users/auth/jwt/login", data=login_data) as response:
...
async def test_shopping_list_serialization(session: aiohttp.ClientSession, token: str, list_id: int):
...
async def test_shared_user_permissions(session: aiohttp.ClientSession, token: str, list_id: int):
    ...
    if not items:
        ...
        status, create_response = await make_authenticated_request(
            session, "POST", f"{BASE_URL}/shopping-lists/{list_id}/items", token, create_data
        )
    ...
    if status == 200:
        updated_item = next((item for item in updated_list['items'] if item['id'] == test_item_id), None)
...
async def test_owner_permissions(session: aiohttp.ClientSession, token: str, list_id: int):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            shared_token = await authenticate_user(session, SHARED_USER_EMAIL, SHARED_USER_PASSWORD)
            print(f"✅ Shared user authenticated: {SHARED_USER_EMAIL}")
            
            # Test 2: Test serialization with shared user
            serialization_ok = await test_shopping_list_serialization(session, shared_token, SHARED_LIST_ID)
            
            # Test 3: Test shared user permissions
            if serialization_ok:
                permissions_ok = await test_shared_user_permissions(session, shared_token, SHARED_LIST_ID)
            else:
            ...
            owner_ok = await test_owner_permissions(session, owner_token, SHARED_LIST_ID)
            
            # Test 6: Test serialization with owner
            owner_serialization_ok = await test_shopping_list_serialization(session, owner_token, SHARED_LIST_ID)
            
            # Final results
            print("\n" + "="*60)
            print("📋 TEST RESULTS SUMMARY")
            print("="*60)
            print(f"✅ Shared user authentication: {'PASS' if shared_token else 'FAIL'}")
            print(f"✅ Serialization (shared user): {'PASS' if serialization_ok else 'FAIL'}")
            ...
            print(f"✅ Serialization (owner): {'PASS' if owner_serialization_ok else 'FAIL'}")
            
            all_passed = all([shared_token, serialization_ok, permissions_ok, owner_token, owner_ok, owner_serialization_ok])
