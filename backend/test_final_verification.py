...
class APIClient:
    ...
    async def login(self, email: str, password: str):
        ...
        async with self.session.post(f"{self.base_url}/api/v1/auth/jwt/login", data=data) as resp:
            if resp.status == 200:
...
async def test_shopping_list_endpoints():
    ...
    async with APIClient(BASE_URL) as owner_client:
        ...
        status, data = await owner_client.request("POST", "/api/v1/shopping-lists/", json=list_data)
        if status != 200:
        ...
        status, data = await owner_client.request("POST", f"/api/v1/shopping-lists/{list_id}/share", json=share_data)
        if status != 200:
        ...
        status, data = await owner_client.request("GET", f"/api/v1/shopping-lists/{list_id}")
        if status != 200:
        ...
        status, item_response = await owner_client.request("POST", f"/api/v1/shopping-lists/{list_id}/items", json=item_data)
        if status != 200:
    ...
    async with APIClient(BASE_URL) as shared_client:
        ...
        status, data = await shared_client.request("GET", f"/api/v1/shopping-lists/{list_id}")
        if status != 200:
        ...
        status, data = await shared_client.request("GET", f"/api/v1/shopping-lists/{list_id}/items")
        if status != 200:
        ...
        status, data = await shared_client.request("PUT", f"/api/v1/items/{item_id}", json=update_data)
        if status != 200:
        ...
        status, data = await shared_client.request("POST", f"/api/v1/shopping-lists/{list_id}/items", json=shared_item_data)
        if status != 200:
        ...
        status, data = await shared_client.request("DELETE", f"/api/v1/items/{shared_item_id}")
        if status != 200:
    ...
    async with APIClient(BASE_URL) as owner_client:
        ...
        status, data = await owner_client.request("DELETE", f"/api/v1/shopping-lists/{list_id}")
        if status != 204:
...
async def test_serialization_issues():
    ...
    async with APIClient(BASE_URL) as client:
        ...
        list_data = {"name": "Serialization Test", "description": "Test for serialization"}
        status, create_response = await client.request("POST", "/api/v1/shopping-lists/", json=list_data)
        if status != 200:
        ...
        status, get_response = await client.request("GET", f"/api/v1/shopping-lists/{list_id}")
        if status != 200:
        ...
        status, item_response = await client.request("POST", f"/api/v1/shopping-lists/{list_id}/items", json=item_data)
        if status != 200:
            print(f"❌ Add item failed: {status} - {item_response}")
            return False
        
        # Get list with items
        status, list_with_items = await client.request("GET", f"/api/v1/shopping-lists/{list_id}")
        if status != 200:
        ...
        share_data = {"email": SHARED_EMAIL}
        status, share_response = await client.request("POST", f"/api/v1/shopping-lists/{list_id}/share", json=share_data)
        if status != 200:
