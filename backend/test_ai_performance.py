...
async def test_uncached_item_performance():
    ...
    for i, item_name in enumerate(test_items, 1):
        ...
        try:
            ...
            category_name = await ai_service.suggest_category_async(item_name, category_names)
            ...
            standardization_result = await ai_service.standardize_and_translate_item_name(item_name)
            translation_time = time.time() - translation_start
            standardized_name = standardization_result.get("standardized_name", item_name)
            translations = standardization_result.get("translations", {})
            print(f"     ✅ Standardized: '{standardized_name}' ({translation_time:.2f}s)")
            ...
            print(f"     Breakdown: Category({category_time:.1f}s) + Translation({translation_time:.1f}s) + Icon({icon_time:.1f}s)")
