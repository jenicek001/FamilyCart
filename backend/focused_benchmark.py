    ...
    async def benchmark_operation(self, provider, provider_name, operation, item):
        ...
        try:
            if operation == "categorization":
                ...
                result = await provider.generate_text(f"Describe '{item}' in one sentence.")
    ...
    async def run_provider_benchmark(self, provider, provider_name):
        ...
        for item in self.test_items:
            ...
            for operation in self.operations:
                result = await self.benchmark_operation(provider, provider_name, operation, item)
                results.append(result)
                
                if result["success"]:
                    print(f"  ✅ {operation}: {result['time']:.3f}s - {result['result']}")
                else:
                    print(f"  ❌ {operation}: {result['time']:.3f}s - {result['error']}")
    ...
    def analyze_results(self, gemini_results, ollama_results):
        ...
        if gemini_results:
            ...
            if gemini_stats:
                ...
                print(f"   Range: {gemini_stats['min_time']:.3f}s - {gemini_stats['max_time']:.3f}s")
        ...
        if ollama_results:
            ...
            if ollama_stats:
                ...
                print(f"   Range: {ollama_stats['min_time']:.3f}s - {ollama_stats['max_time']:.3f}s")
    ...
    async def run(self):
        ...
        if settings.GOOGLE_API_KEY:
            try:
                ...
                gemini_results = await self.run_provider_benchmark(gemini_provider, "Gemini")
            except Exception as e:
        ...
        try:
            ...
            ollama_results = await self.run_provider_benchmark(ollama_provider, "Ollama")
        except Exception as e:
