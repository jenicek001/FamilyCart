...
def analyze_benchmark_results(json_file):
    ...
    for model_key, model_data in data.items():
        ...
        english_times = [r.get('response_time', 0) for r in english_tests if r.get('response_time')]
        czech_times = [r.get('response_time', 0) for r in czech_tests if r.get('response_time')]
        
        avg_english_time = sum(english_times) / len(english_times) if english_times else 0
        ...
        model_summaries[model_name] = {
            ...
            'accuracy_total': correct_total / total_tests * 100 if total_tests > 0 else 0,
            'accuracy_english': correct_english / len(english_tests) * 100 if english_tests else 0,
            'accuracy_czech': correct_czech / len(czech_tests) * 100 if czech_tests else 0,
    ...
    for model_name, summary in model_summaries.items():
        print(f"\n🤖 {model_name}")
        print(f"   Tests: {summary['total_tests']} total ({summary['english_tests']} EN, {summary['czech_tests']} CZ)")
        print(f"   Overall Accuracy: {summary['accuracy_total']:.1f}% ({summary['correct_total']}/{summary['total_tests']})")
        print(f"   English Accuracy: {summary['accuracy_english']:.1f}% ({summary['correct_english']}/{summary['english_tests']})")
        print(f"   Czech Accuracy: {summary['accuracy_czech']:.1f}% ({summary['correct_czech']}/{summary['czech_tests']})")
    ...
    sorted_models = sorted(model_summaries.items(), key=lambda x: x[1]['accuracy_total'], reverse=True)
    
    for model_name, summary in sorted_models:
        print(f"{model_name:<20} {summary['accuracy_total']:<10.1f}% {summary['accuracy_english']:<10.1f}% {summary['accuracy_czech']:<10.1f}%")
    ...
    sorted_models = sorted(model_summaries.items(), key=lambda x: x[1]['avg_response_time'])
    
    for model_name, summary in sorted_models:
        print(f"{model_name:<20} {summary['avg_response_time']:<12.3f}s {summary['avg_english_time']:<12.3f}s {summary['avg_czech_time']:<12.3f}s")
    ...
    print(f"🎯 Best Overall Accuracy: {best_accuracy[0]} ({best_accuracy[1]['accuracy_total']:.1f}%)")
    print(f"🇺🇸 Best English Accuracy: {best_english[0]} ({best_english[1]['accuracy_english']:.1f}%)")
    print(f"🇨🇿 Best Czech Accuracy: {best_czech[0]} ({best_czech[1]['accuracy_czech']:.1f}%)")
    ...
    print(f"    ({best_balanced_model['accuracy_total']:.1f}% accuracy, {best_balanced_model['avg_response_time']:.3f}s response)")
    
    # Fallback recommendation
    print(f"\n🔄 Fallback Configuration Recommendation:")
    print(f"   Primary: {best_accuracy[0]} (best accuracy: {best_accuracy[1]['accuracy_total']:.1f}%)")
    print(f"   Fallback: {fastest[0]} (fastest: {fastest[1]['avg_response_time']:.3f}s)")
    ...
    for model_key, model_data in data.items():
        ...
        if errors:
            ...
            for error in errors[:5]:  # Show first 5 errors
                print(f"   • '{error.get('item')}' ({error.get('language')}) -> got '{error.get('actual')}', expected '{error.get('expected')}'")
            if len(errors) > 5:
