...
async def test_with_proper_cache_setup():
    ...
    print(f"✅ Cache service setup complete. Client: {cache_service.redis_client is not None}")
    ...
    print(f"  Improvement: {((first_call_time - second_call_time) / first_call_time * 100):.1f}%")
...
async def test_parallel_vs_sequential():
    ...
    category_name = await ai_service.suggest_category_async(test_item, category_names)
    standardization_result = await ai_service.standardize_and_translate_item_name(test_item)
    ...
    standardization_result_2, icon_name_2 = await asyncio.gather(translation_task, icon_task)
