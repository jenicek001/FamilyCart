...
async def test_frontend_sharing_integration():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Share successful! List has {len(shared_list.get('members', []))} members")
            
            # Verify the returned structure matches frontend expectations
            required_fields = ['id', 'name', 'owner_id', 'members', 'items']
            missing_fields = [field for field in required_fields if field not in shared_list]
        ...
        if response.status_code == 200:
            ...
            missing_user_fields = [field for field in user_fields if field not in user_data]
