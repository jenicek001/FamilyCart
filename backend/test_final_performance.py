...
async def test_real_api_performance():
    ...
    print(
        "   1. First time adding an item (cache miss) - should be faster with parallel AI"
    )
    ...
    test_items = [
        ...
        {
            "name": "unique_test_item_12345",
            "quantity": 1,
            "expected": "uncached (parallel AI)",
        },
    ]
    ...
    try:
        result = subprocess.run(
            [
                ...
                "redis-cli",
                "-a",
                "asdlkhjHJhjkjkha23897234",
                "KEYS",
                "*suggestion*",
            ],
            capture_output=True,
            text=True,
            cwd="/home/honzik/GitHub/FamilyCart/FamilyCart",
        )
        ...
        print("💰 Cost Impact:")
        print(
            f"   - {len(cache_keys)} cached entries = {len(cache_keys)} avoided AI calls"
        )
        print(f"   - Each avoided call saves ~$0.002 + 1-5 seconds")
        print(
            f"   - Total savings: ${len(cache_keys) * 0.002:.3f} + {len(cache_keys)} seconds"
        )
