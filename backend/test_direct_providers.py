...
async def test_direct_providers():
    try:
        ...
        try:
            ollama_result = (
                await fallback_ai_service.fallback_provider.suggest_category_async(
        ...
        try:
            gemini_result = (
                await fallback_ai_service.primary_provider.suggest_category_async(
                    "sůl do myčky", ["Cleaning", "Food"]
                )
            )
        ...
        try:
            fallback_result = await fallback_ai_service.suggest_category_async(
                "sůl do myčky", ["Cleaning", "Food"]
            )
