...
CZECH_ITEMS = [
    ...
    {"name": "rohlíky", "expected_category": "Pantry", "description": "fresh bread rolls"},  # Pantry is correct for bread
    {"name": "jablka", "expected_category": "Produce", "description": "red apples"},  # Produce is correct for fruits
...
class EndToEndTester:
    ...
    def get_user_info(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            self.log(f"Failed to get user info: {response.status_code} - {response.text}")
            return False
    ...
    def create_shopping_list(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            self.log(f"Failed to create shopping list: {response.status_code} - {response.text}")
            return False
    ...
    def verify_categorization(self, item: Dict[str, Any], expected_category: str) -> bool:
        ...
        if actual_category == expected_category:
            ...
            self.log(f"❌ Categorization incorrect: {item['name']} -> {actual_category} (expected: {expected_category})")
            return False
    ...
    def cleanup(self):
        ...
        if self.shopping_list_id:
            ...
            response = self.session.delete(f"{API_V1}/shopping-lists/{self.shopping_list_id}")
            if response.status_code == 200:
    ...
    async def run_test(self) -> bool:
        ...
        try:
            ...
            for item_data in CZECH_ITEMS:
                ...
                if item and self.verify_categorization(item, item_data["expected_category"]):
