...
async def test_share_endpoint():
    ...
    login_data = {"email": "test1@example.com", "password": "testpassword123"}

    share_data = {"email": "test2@example.com"}  # Share with another test user
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.post(
                f"{base_url}/api/v1/auth/jwt/login", data=login_data
            ) as response:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    response_data = await response.json()
                    print(
                        f"   Shared list has {len(response_data.get('items', []))} items"
                    )
