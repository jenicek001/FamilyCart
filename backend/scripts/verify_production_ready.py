...
def check_categories():
    ...
    with engine.connect() as conn:
        ...
        if count < 17:
            ...
            print(
                "Run: docker exec familycart-backend-dev python scripts/seed_categories.py"
            )
            return False
...
def check_gemini_config():
    ...
    print(
        f"✓ GEMINI_API_KEY: {gemini_key[:10]}...{gemini_key[-4:]} (length: {len(gemini_key)})"
    )
...
def check_items_categorization():
    ...
    with engine.connect() as conn:
        ...
        if uncategorized_count == total_items and total_items > 0:
            ...
            print(
                "Check backend logs: docker logs familycart-backend-dev | grep -i gemini"
            )
