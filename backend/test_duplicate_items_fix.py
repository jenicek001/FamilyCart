...
async def test_duplicate_items_fix():
    ...
    lists_response = requests.get(
        f"{API_BASE_URL}/api/v1/shopping-lists/", headers=headers
    )
    ...
    try:
        async with websockets.connect(ws_url) as websocket:
            ...
            async def listen_for_messages():
                while True:
                    try:
                        ...
                        if (
                            data.get("type") == "item_change"
                            and data.get("event_type") == "created"
                        ):
                            item_creation_events.append(data)
                            print(
                                f"🆕 Item creation event received: {data.get('item', {}).get('name')}"
                            )
            ...
            print(
                f"✅ Item created via API: {created_item['name']} (ID: {created_item['id']})"
            )
            ...
            if len(item_creation_events) == 0:
                ...
                if event_item.get("name") == test_item_name:
                    print(
                        "✅ Exactly one item creation event received - no duplicates!"
                    )
                    print(
                        f"   • Event item: {event_item.get('name')} (ID: {event_item.get('id')})"
                    )
                    return True
                else:
                ...
                print(
                    f"❌ Multiple item creation events received - duplicates detected!"
                )
                for i, event in enumerate(item_creation_events):
                    event_item = event.get("item", {})
                    print(
                        f"   • Event {i+1}: {event_item.get('name')} (ID: {event_item.get('id')})"
                    )
                return False
