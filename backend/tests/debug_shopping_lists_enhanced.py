...
def debug_shopping_lists_api():
    ...
    for endpoint in endpoints_to_test:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"{Colors.WARNING}Request completed but returned non-200 status code.{Colors.ENDC}")
    ...
    # Test both with and without trailing slash
    endpoints_to_test = [
        {"url": f"{base_url}/api/v1/shopping-lists", "name": "Shopping lists (no trailing slash)"},
        {"url": f"{base_url}/api/v1/shopping-lists/", "name": "Shopping lists (trailing slash)"},
    ]
    ...
    try:
        ...
        print(f"Access-Control-Allow-Methods: {options_response.headers.get('Access-Control-Allow-Methods', 'Not present')}")
