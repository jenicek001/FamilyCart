    ...
    endpoints_to_test = [
        {
            ...
            "name": "No trailing slash (default)",
        },
        {
            "url": f"{base_url}/api/v1/shopping-lists/",
            "name": "With trailing slash (default)",
        },
    ]
    ...
    for endpoint in endpoints_to_test:
        try:
            ...
            response = requests.get(
                endpoint["url"], headers=headers, allow_redirects=False
            )
            ...
            if 300 <= response.status_code < 400:  # Redirect status code
                print(
                    f"{Colors.WARNING}Redirect detected! Location: {response.headers.get('Location')}{Colors.ENDC}"
                )
            elif response.status_code == 200:
                print(
                    f"{Colors.OKGREEN}Direct 200 response (no redirect needed){Colors.ENDC}"
                )
                try:
                    json_response = response.json()
                    print(f"Data: {json.dumps(json_response, indent=2)}")
