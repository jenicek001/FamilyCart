...
class TestListConnectionManager:
    ...
    async def test_authenticate_user_valid_token(self):
        ...
        token_data = {"sub": "1", "email": "test@example.com", "aud": "fastapi-users:auth"}
    ...
    async def test_authenticate_user_invalid_array_audience(self):
        ...
        token_data = {"sub": "1", "email": "test@example.com", "aud": ["wrong-audience"]}
        token = jwt.encode(token_data, settings.SECRET_KEY, algorithm="HS256")
    ...
    async def test_authenticate_user_multiple_audiences(self):
        ...
        token_data = {"sub": "1", "email": "test@example.com", "aud": ["other-service", "fastapi-users:auth", "another-service"]}
        token = jwt.encode(token_data, settings.SECRET_KEY, algorithm="HS256")
    ...
    async def test_broadcast_item_change(self):
        ...
        manager.websocket_registry[mock_websocket] = {"user_id": "user123", "list_id": 1}
...
@pytest.mark.asyncio
async def test_ping_pong_functionality(connection_manager_with_connection, mock_websocket):
    ...
    await manager.send_to_websocket(mock_websocket, {
        "type": "pong",
