...
response = requests.post(f"{API_URL}/shopping-lists/", json=list_data, headers=headers)
print_response(response)
...
response = requests.get(f"{API_URL}/shopping-lists/", headers=headers)
print_response(response)
...
if list_id:
    ...
    response = requests.get(f"{API_URL}/shopping-lists/{list_id}/", headers=headers)
    print_response(response)
...
print(
    "If you see 307 redirects followed by 401 errors, it means authorization headers are being lost during redirects."
)
print(
    "The frontend should always use URLs with trailing slashes for consistency with FastAPI."
)
