...
async def test_sprint5_sharing_integration():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{base_url}/api/v1/auth/register", json=owner_data
        )
        ...
        response = await client.post(
            f"{base_url}/api/v1/auth/register", json=member_data
        )
        if response.status_code == 201:
            print("✅ Member user created successfully")
        else:
            print(
                f"❌ Member creation failed: {response.status_code} - {response.text}"
            )
            return
        ...
        response = await client.post(
            f"{base_url}/api/v1/shopping-lists", json=list_data, headers=owner_headers
        )
        ...
        response = await client.get(
            f"{base_url}/api/v1/shopping-lists/{list_id}", headers=member_headers
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"❌ Item creation by member failed: {response.status_code} - {response.text}"
            )
            return
        ...
        response = await client.get(
            f"{base_url}/api/v1/shopping-lists/{list_id}/items", headers=owner_headers
        )
        ...
        if response.status_code == 200:
            ...
            if items and len(items) > 0:
                print(
                    f"✅ Owner can see {len(items)} item(s) including member's additions"
                )
            else:
                print("⚠️ No items found in shared list")
        else:
            print(
                f"❌ Owner item access failed: {response.status_code} - {response.text}"
            )
            return
        ...
        response = await client.post(
            f"{base_url}/api/v1/auth/register", json=outsider_data
        )
        if response.status_code != 201:
        ...
        response = await client.get(
            f"{base_url}/api/v1/shopping-lists/{list_id}", headers=outsider_headers
        )
        ...
        if response.status_code == 403:
            ...
            print(
                f"❌ Security issue: Outsider access not blocked properly: {response.status_code}"
            )
            return
