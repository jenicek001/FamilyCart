...
class ModelBenchmark:
    ...
    async def test_model(self, model_name: str, test_items: List[Dict]) -> Dict:
        ...
        for item_data in test_items:
            ...
            try:
                ...
                results["categorization_results"].append({
                    ...
                    "actual": category,
                    "correct": self.is_category_correct(category, expected_category),
                    "response_time": response_time
                })
                results["response_times"].append(response_time)
                
                status = "✅" if self.is_category_correct(category, expected_category) else "❌"
                print(f"      {status} '{item_name}' [{language}] -> '{category}' ({response_time:.3f}s)")
        ...
        english_correct = sum(1 for r in results["categorization_results"] 
                            if r["language"] == "english" and r["correct"])
        english_total = sum(1 for r in results["categorization_results"] 
                          if r["language"] == "english")
        
        czech_correct = sum(1 for r in results["categorization_results"] 
                          if r["language"] == "czech" and r["correct"])
        czech_total = sum(1 for r in results["categorization_results"] 
                        if r["language"] == "czech")
        
        results["accuracy_scores"] = {
            "english_accuracy": english_correct / english_total if english_total > 0 else 0,
            "czech_accuracy": czech_correct / czech_total if czech_total > 0 else 0,
            "overall_accuracy": (english_correct + czech_correct) / (english_total + czech_total) if (english_total + czech_total) > 0 else 0
        }
    ...
    def is_category_correct(self, actual: str, expected: str) -> bool:
        ...
        for main_cat, alternatives in category_mappings.items():
            if (main_cat in actual_clean and any(alt in expected_clean for alt in alternatives)) or \
               (main_cat in expected_clean and any(alt in actual_clean for alt in alternatives)):
                return True
    ...
    async def benchmark_against_gemini(self, test_items: List[Dict]) -> Dict:
        ...
        for item_data in test_items:
            ...
            try:
                start_time = time.time()
                category = await self.gemini_provider.suggest_category_async(item_name, [])
                response_time = time.time() - start_time
    ...
    def print_summary(self, all_results: Dict):
        ...
        sorted_models = sorted(all_results.items(), key=lambda x: x[1].get("accuracy_scores", {}).get("overall_accuracy", 0), reverse=True)
        
        for model_name, results in sorted_models:
            if "error" in results:
                print(f"{model_name:<20} {'ERROR':<8} {'ERROR':<8} {'ERROR':<8} {'ERROR':<10} {'N/A':<7}")
                continue
            ...
            print(f"{model_name:<20} {eng_acc:<8} {cz_acc:<8} {overall_acc:<8} {avg_time:<10} {errors:<7}")
        
        # Performance ranking
        print(f"\n🏆 PERFORMANCE RANKINGS:")
        print(f"   Accuracy: {sorted_models[0][0]} ({sorted_models[0][1].get('accuracy_scores', {}).get('overall_accuracy', 0)*100:.1f}%)")
        
        speed_sorted = sorted(all_results.items(), key=lambda x: x[1].get("avg_response_time", float('inf')))
        print(f"   Speed: {speed_sorted[0][0]} ({speed_sorted[0][1].get('avg_response_time', 0):.3f}s)")
        
        # Language-specific performance
        print(f"\n🌍 LANGUAGE-SPECIFIC PERFORMANCE:")
        print(f"   English Best: {max(all_results.items(), key=lambda x: x[1].get('accuracy_scores', {}).get('english_accuracy', 0))[0]}")
        print(f"   Czech Best: {max(all_results.items(), key=lambda x: x[1].get('accuracy_scores', {}).get('czech_accuracy', 0))[0]}")
        ...
        if best_overall[0] == best_speed[0]:
            print(f"   🥇 Overall Winner: {best_overall[0]} (best accuracy + good speed)")
        else:
            print(f"   🥇 Best Accuracy: {best_overall[0]} ({best_overall[1].get('accuracy_scores', {}).get('overall_accuracy', 0)*100:.1f}%)")
            print(f"   🥇 Best Speed: {best_speed[0]} ({best_speed[1].get('avg_response_time', 0):.3f}s)")
            print(f"   💡 Consider {best_overall[0]} for production (accuracy priority)")
...
async def main():
    ...
    test_items = [
        # English items
        {"name": "organic apples", "expected_category": "Produce", "language": "english"},
        {"name": "whole milk", "expected_category": "Dairy", "language": "english"},
        {"name": "sourdough bread", "expected_category": "Bakery", "language": "english"},
        {"name": "chicken breast", "expected_category": "Meat", "language": "english"},
        {"name": "dish soap", "expected_category": "Household", "language": "english"},
        {"name": "orange juice", "expected_category": "Beverages", "language": "english"},
        ...
        {"name": "pomerančový džus", "expected_category": "Beverages", "language": "czech"},
    ...
    print(f"📋 Test items: {len(test_items)} ({len([i for i in test_items if i['language'] == 'english'])} English, {len([i for i in test_items if i['language'] == 'czech'])} Czech)")
