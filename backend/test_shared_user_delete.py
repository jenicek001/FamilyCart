...
def test_shared_user_item_deletion():
    ...
    for list_item in lists_data:
        print(
            f"  - List {list_item['id']}: {list_item['name']} (owner: {list_item.get('owner_email', 'N/A')})"
        )
    ...
    list_detail_response = requests.get(
        f"{base_url}/api/v1/shopping-lists/{list_id}", headers=headers
    )
    ...
    if not items:
        ...
        new_item_data = {"name": "Test item for deletion", "quantity": 1}
        ...
        item_id = items[0]["id"]
        print(
            f"✅ Will try to delete existing item with ID: {item_id} ('{items[0]['name']}')"
        )
    ...
    delete_response = requests.delete(
        f"{base_url}/api/v1/items/{item_id}", headers=headers
    )
