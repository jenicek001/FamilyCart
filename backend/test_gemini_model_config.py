...
try:
    ...
    if settings.GEMINI_MODEL_NAME == expected_model:
        ...
        print(
            f"❌ Model configuration incorrect. Expected: {expected_model}, Got: {settings.GEMINI_MODEL_NAME}"
        )

except ImportError as e:
    print(f"❌ Import failed: {e}")
    print(
        "💡 Make sure you're running this from the backend directory with: poetry run python test_gemini_model_config.py"
    )
except Exception as e:
