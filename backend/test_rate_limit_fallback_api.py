...
def test_ai_status(token):
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            f"AI status check failed: {response.status_code} - {response.text}"
        )
...
def test_categorization(token, item):
    ...
    response = requests.post(
        f"{base_url}/ai/categorize-item", json=data, headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            f"Categorization failed: {response.status_code} - {response.text}"
        )
