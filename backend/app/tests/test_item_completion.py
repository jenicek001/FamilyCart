...
@pytest.mark.asyncio
async def test_update_item_completion_status(
    client: AsyncClient, token_header: dict, test_item: Item, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_update_item_completion_back_to_false(
    client: AsyncClient, token_header: dict, test_item: Item, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_update_item_with_multiple_fields_including_completion(
    client: AsyncClient, token_header: dict, test_user: dict, test_item: Item
):
    """Test updating multiple fields including completion status."""
    response = await client.put(
        f"/api/v1/items/{test_item.id}",
        json={"name": "Updated Item Name", "quantity": "2", "is_completed": True},
        headers=token_header,
    )
...
@pytest.mark.asyncio
async def test_update_nonexistent_item_completion(
    client: AsyncClient, token_header: dict
):
    """Test updating completion status of non-existent item."""
    response = await client.put(
        "/api/v1/items/99999", json={"is_completed": True}, headers=token_header
    )
...
@pytest.mark.asyncio
async def test_unauthorized_update_item_completion(
    client: AsyncClient, test_item: Item
):
    """Test updating item completion without authorization."""
    response = await client.put(
        f"/api/v1/items/{test_item.id}", json={"is_completed": True}
    )
...
@pytest.mark.asyncio
async def test_update_item_completion_wrong_owner(
...
# Edge case tests
@pytest.mark.asyncio
async def test_update_item_completion_with_invalid_data_type(
    client: AsyncClient, token_header: dict, test_item: Item
):
...
@pytest.mark.asyncio
async def test_update_item_completion_preserves_other_fields(
    client: AsyncClient, token_header: dict, test_item: Item
):
