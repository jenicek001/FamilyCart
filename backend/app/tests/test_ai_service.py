...
def ai_service_mocker():
    ...
    """
    with patch('google.generativeai.GenerativeModel', new_callable=MagicMock) as mock_genai_model, \
         patch('app.services.ai_service.cache_service', spec=CacheService) as mock_cache_service, \
         patch('app.services.ai_service.crud_category', spec=CRUDCategory) as mock_crud_category:
...
async def test_suggest_category_new_item(ai_service: AIService, mock_db_session: MagicMock):
    """
    ...
    # Assertions
    ai_service.mocks['crud_category'].get_multi.assert_called_once_with(mock_db_session, limit=1000)
    ...
    ai_service.mocks['cache'].set.assert_called_once_with(cache_key, "Bakery", expire=3600 * 24 * 180)
    assert suggested_category == "Bakery"

async def test_suggest_category_cached_item(ai_service: AIService, mock_db_session: MagicMock):
    """
...
async def test_suggest_icon_new_item(ai_service: AIService):
    ...
    ai_service.mocks['model'].generate_content_async.assert_called_once()
    ai_service.mocks['cache'].get.assert_called_once_with(cache_key)
    ai_service.mocks['cache'].set.assert_called_once_with(cache_key, "leaf", expire=3600 * 24 * 180)
...
async def test_standardize_and_translate_item_name_new_item(ai_service: AIService):
    ...
    ai_service.mocks['model'].generate_content_async.assert_called_once()
    ai_service.mocks['cache'].get.assert_called_once_with(cache_key)
    ai_service.mocks['cache'].set.assert_called_once_with(cache_key, json.dumps(response_data), expire=3600 * 24 * 180)
