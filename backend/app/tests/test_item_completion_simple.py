...
@pytest.mark.asyncio
async def test_update_item_completion_status_simple(
    client: AsyncClient, test_db: AsyncSession, test_user: dict, token_header: dict
):
    ...
    response = await client.put(
        f"/api/v1/items/{item_id}", json={"is_completed": True}, headers=token_header
    )
...
@pytest.mark.asyncio
async def test_update_item_completion_back_to_false_simple(
    client: AsyncClient, test_db: AsyncSession, test_user: dict, token_header: dict
):
    ...
    response = await client.put(
        f"/api/v1/items/{item_id}", json={"is_completed": False}, headers=token_header
    )
