...
@pytest.mark.asyncio
async def test_audit_logging_for_item_completion(
    ...
    with patch("app.api.v1.endpoints.items.logger") as mock_logger:
        ...
        response = await client.put(
            f"/api/v1/items/{item_id}", json=update_data, headers=token_header
        )
...
@pytest.mark.asyncio
async def test_no_audit_log_when_completion_status_unchanged(
    client: AsyncClient, test_db: AsyncSession, test_user: dict, token_header: dict
):
...
@pytest.mark.asyncio
async def test_no_audit_log_when_setting_same_completion_status(
    client: AsyncClient, test_db: AsyncSession, test_user: dict, token_header: dict
):
