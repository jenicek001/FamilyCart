...
class TestSharingEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_share_list_only_owner_can_share(self, test_db: AsyncSession, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_share_list_user_not_found(self, test_db: AsyncSession, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_share_list_already_shared(self, test_db: AsyncSession, client: AsyncClient):
...
class TestItemPermissions:
    ...
    @pytest.mark.asyncio
    async def test_shared_member_can_add_items(self, test_db: AsyncSession, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_shared_member_can_read_items(self, test_db: AsyncSession, client: AsyncClient):
        ...
        # Mock authentication
        with patch('app.api.v1.endpoints.shopping_lists.current_user', return_value=member):
            response = await client.get(f"/api/v1/shopping_lists/{shopping_list.id}/items")
...
class TestNotificationService:
    ...
    def test_send_list_invitation_email(self):
        ...
        with patch('app.services.notification_service.send_list_invitation_email') as mock_send:
