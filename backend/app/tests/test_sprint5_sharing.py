...
class TestSharingPermissions:
    ...
    async def test_get_shopping_list_owner_access(self, test_db: AsyncSession):
        ...
        owner = User(email="owner@test.com", hashed_password="hashed", is_active=True, is_verified=True)
        test_db.add(owner)
    ...
    async def test_get_shopping_list_shared_member_access(self, test_db: AsyncSession):
        ...
        owner = User(email="owner@test.com", hashed_password="hashed", is_active=True, is_verified=True)
        member = User(email="member@test.com", hashed_password="hashed", is_active=True, is_verified=True)
    ...
    @pytest.mark.asyncio
    async def test_get_shopping_list_unauthorized_access(self, async_session: AsyncSession):
        """Test that unauthorized users cannot access lists."""
        from fastapi import HTTPException
        
        # Create users
        owner = User(email="owner@test.com", hashed_password="hashed", is_active=True, is_verified=True)
        outsider = User(email="outsider@test.com", hashed_password="hashed", is_active=True, is_verified=True)
    ...
    async def test_get_shopping_list_nonexistent(self, async_session: AsyncSession):
        ...
        user = User(email="user@test.com", hashed_password="hashed", is_active=True, is_verified=True)
...
class TestSharingEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_share_list_success(self, async_session: AsyncSession, client: TestClient):
        """Test successful list sharing."""
        # Create owner and target user
        owner = User(email="owner@test.com", hashed_password="hashed", is_active=True, is_verified=True)
        target_user = User(email="target@test.com", hashed_password="hashed", is_active=True, is_verified=True)
        ...
        with patch('app.api.v1.endpoints.shopping_lists.current_user', return_value=owner), \
             patch('app.api.v1.endpoints.shopping_lists.websocket_service.notify_list_shared', new_callable=AsyncMock), \
             patch('app.api.v1.endpoints.shopping_lists.send_list_invitation_email', new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_share_list_only_owner_can_share(self, async_session: AsyncSession, client: TestClient):
        ...
        with patch('app.api.v1.endpoints.shopping_lists.current_user', return_value=member):
            response = client.post(
                f"/api/v1/shopping_lists/{shopping_list.id}/share",
                json={"email": target_user.email}
            )
    ...
    @pytest.mark.asyncio
    async def test_share_list_user_not_found(self, async_session: AsyncSession, client: TestClient):
    ...
    @pytest.mark.asyncio
    async def test_share_list_already_shared(self, async_session: AsyncSession, client: TestClient):
...
class TestItemPermissions:
    ...
    @pytest.mark.asyncio
    async def test_shared_member_can_add_items(self, async_session: AsyncSession, client: TestClient):
        ...
        with patch('app.api.v1.endpoints.shopping_lists.current_user', return_value=member), \
             patch('app.api.v1.endpoints.shopping_lists.ai_service.suggest_category_async', new_callable=AsyncMock, return_value="Groceries"), \
             patch('app.api.v1.endpoints.shopping_lists.ai_service.standardize_and_translate_item_name', new_callable=AsyncMock, return_value={"standardized_name": "Milk", "translations": {}}), \
             patch('app.api.v1.endpoints.shopping_lists.ai_service.suggest_icon', new_callable=AsyncMock, return_value="milk"), \
             patch('app.api.v1.endpoints.shopping_lists.websocket_service.notify_item_created', new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_shared_member_can_read_items(self, async_session: AsyncSession, client: TestClient):
