...
async def test_czech_categorization():
    ...
    test_items = [
        ...
        {"name": "maso na gril", "expected_category": "meat", "english": "grilling meat"},
    ]
    ...
    for item in test_items:
        try:
            ...
            suggested_category = await ai_service.suggest_category(item["name"], mock_db)
            ...
            print(f"   {status} Suggested: '{suggested_category}' | Expected: '{item['expected_category']}'")
    ...
    if correct_count < total_count:
        ...
        for result in results:
            if not result["correct"]:
                print(f"   • {result['item']} ({result['english']}) -> '{result['suggested']}' (expected: '{result['expected']}')")
        print()
