...
def test_shared_user_item_permissions():
    ...
    uncompleted_items = [
        ...
        if not item.get("is_completed", False)
    ]
    ...
    if update_response.status_code == 200:
        ...
        print(
            f"   Item '{updated_item['name']}' marked as completed: {updated_item['is_completed']}"
        )
    else:
    ...
    read_response = requests.get(f"{base_url}/api/v1/items/{item_id}", headers=headers)
