...
class WebSocketLoadTest:
                ...
                while True:
                    try:
                        ...
                        if data.get('type') in ['item_change', 'list_change']:
                            ...
                            print(f"📨 Connection {connection_id} received: {data.get('type')}")
    ...
    async def run_load_test(self, list_id: int, token: str):
        ...
        for i in range(self.num_connections):
            task = asyncio.create_task(
                self.simulate_connection(i, list_id, token)
            )
    ...
    def print_results(self):
        ...
        success_rate = (self.num_connections - len(self.errors)) / self.num_connections * 100
    ...
    async def test_connection(connection_id: int):
        ...
        try:
            ...
            try:
                ...
                print(f"✅ Connection {connection_id}: Auth rejected in {response_time:.3f}s (expected)")
                return True
    ...
    try:
        ...
        try:
            ...
            if e.status_code in [1008, 1003]:  # Auth failure codes
                print(f"✅ Endpoint reachable, auth working (response in {response_time:.3f}s)")
                return True
            else:
