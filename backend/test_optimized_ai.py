...
class OptimizedAIService:
    ...
    async def suggest_category_optimized(self, item_name: str, category_names: list[str]) -> str:
        """
        ...
        if cached_category:
            print(f"💨 Cache hit for category suggestion: {item_name} -> {cached_category}")
...
Return only the category name."""
        ...
        try:
            ...
            print(f"⚡ AI call completed in {duration:.3f}s: {item_name} -> {suggested_category}")
            
            # Cache for 6 months
            await cache_service.set(cache_key, suggested_category, expire=3600 * 24 * 180)
    ...
    async def suggest_icon_optimized(self, item_name: str, category_name: str) -> str:
        ...
        if cached_icon:
            print(f"💨 Cache hit for icon suggestion: {item_name}/{category_name} -> {cached_icon}")
            return cached_icon
...
Return only the icon name."""
        ...
        try:
            ...
            print(f"⚡ Icon AI call completed in {duration:.3f}s: {item_name} -> {suggested_icon}")
...
async def test_optimized_performance():
    ...
    category_names = ["Produce", "Dairy", "Meat", "Pantry", "Frozen", "Beverages", "Snacks", "Personal Care", "Household", "Uncategorized"]
    
    print(f"\n📊 Performance Test Results:")
    print(f"{'Item':<20} {'Cache':<8} {'Category Time':<15} {'Icon Time':<12} {'Total Time':<12} {'Category':<12}")
    print("-" * 85)
    ...
    for item in test_items:
        ...
        cat_start = time.time()
        category = await optimized_service.suggest_category_optimized(item, category_names)
        ...
        print(f"{item:<20} {cache_status:<8} {cat_time:<15.3f} {icon_time:<12.3f} {overall_time:<12.3f} {category:<12}")
