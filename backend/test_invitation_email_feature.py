...
async def create_test_shopping_list(client: httpx.AsyncClient, token: str) -> int:
    ...
    response = await client.post(
        f"{API_BASE}/shopping-lists/", json=list_data, headers=headers
    )
...
async def test_share_with_nonexistent_user(
    client: httpx.AsyncClient, token: str, list_id: int
) -> bool:
    """Test sharing with a non-existent user - should send invitation email."""
    share_data = {"email": NON_EXISTENT_EMAIL}

    headers = {"Authorization": f"Bearer {token}"}
    response = await client.post(
        f"{API_BASE}/shopping-lists/{list_id}/share", json=share_data, headers=headers
    )
    ...
    if response.status_code == 200:
        print(
            "✅ SUCCESS: Share endpoint returned 200 (invitation email should be sent)"
        )
        return True
...
async def test_share_with_existing_user(
    client: httpx.AsyncClient, token: str, list_id: int
) -> bool:
    """Test sharing with an existing user - should work normally."""
    share_data = {"email": TEST_USER["email"]}  # Share with self (existing user)
...
async def cleanup_test_list(client: httpx.AsyncClient, token: str, list_id: int):
    ...
    response = await client.delete(
        f"{API_BASE}/shopping-lists/{list_id}", headers=headers
    )
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not token:
            print(
                "❌ Login failed. Make sure the test user exists and the backend is running."
            )
            return
        ...
        success_nonexistent = await test_share_with_nonexistent_user(
            client, token, list_id
        )
        ...
        print("🏁 TEST RESULTS:")
        print(
            f"   Non-existent user invitation: {'✅ PASS' if success_nonexistent else '❌ FAIL'}"
        )
        print(
            f"   Existing user sharing: {'✅ PASS' if success_existing else '❌ FAIL'}"
        )
