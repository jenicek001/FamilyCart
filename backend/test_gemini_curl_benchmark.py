...
Category:""",
                ...
                if result.returncode == 0:
                    try:
                        ...
                        if "candidates" in response_data:
                            generated_text = response_data["candidates"][0]["content"][
                                "parts"
                            ][0]["text"].strip()
    ...
    if results:
        ...
        for i, result in enumerate(results[:5]):  # Top 5
            ...
            print(
                f"{i+1}. {result['model']} + {result['prompt']}: {result['duration']:.3f}s {status}"
            )
        ...
        print(
            f"   Improvement vs slowest: {(results[-1]['duration'] / best['duration']):.1f}x faster"
        )
